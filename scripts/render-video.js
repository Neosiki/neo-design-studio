...
(async () => {
  ...
  if (hasReady) {
    // 두 번째 방어선: 애니메이션 time을 적극적으로 0으로 초기화——HTML이 starter tick 템플릿을 엄격히 따르지 않는 경우에 대비
    // 상황(예: lastTick이 performance.now()를 사용해 글꼴 로드 시간이 첫 프레임 dt에 포함되는 경우)
    // 자세한 내용은 references/animation-pitfalls.md §12 참조
    ...
    if (seekCorrected) {
      // 두 번의 rAF를 기다려 seek가 적용되어 t=0 화면을 렌더링하게 함
