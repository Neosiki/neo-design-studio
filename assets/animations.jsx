 ...
 *
 * Stage + Sprite 모드，Remotion을 참고했지만 경량화。
 *
 * 내보내기（window.Animations에 연결）：
 * - Stage: 전체 애니메이션 컨테이너，시간+제어 제공
 * - Sprite: 시간 조각，start/end 내에서 표시，로컬 진행률 제공
 * - useTime(): 전역 시간을 읽음（초）
 * - useSprite(): 로컬 진행률을 읽음 {t: 0→1, elapsed: seconds, duration: seconds}
 ...
 *   </Stage>
 *
 * Sprite 하위 컴포넌트에서 useSprite()로 현재 조각 진행률을 읽습니다。
 */
...
(function() {
  ...
  const Easing = {
    ...
    // expoOut: Anthropic-level 주 easing (cubic-bezier(0.16, 1, 0.3, 1))
    // 빠른 시작 + 느린 제동，숫자 요소에 물리적 무게감 부여
    expoOut: t => t === 1 ? 1 : 1 - Math.pow(2, -10 * t),
    // overshoot: 탄성이 있는 toggle/버튼 팝업 (cubic-bezier(0.34, 1.56, 0.64, 1))
    overshoot: t => {
