/**
 * <deck-stage> — HTML 슬라이드 외피 web component
 *
 * 제공 기능:
 ...
 * - 좌우 클릭 영역으로 탐색
 * - 슬라이드 카운터 (현재/총)
 * - localStorage에 현재 슬라이드 영구 저장
 * - Speaker notes postMessage (외부 렌더링 지원)
 * - 해시 네비게이션 (#slide-5 — 5번째 슬라이드로 이동)
 * - Print-to-PDF 지원 (Cmd+P / Ctrl+P — 슬라이드당 한 페이지)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Shadow DOM 먼저 렌더링(자식 노드와 독립적이며 parser 시점의 영향을 받지 않음)
      this._render();

      // 방어: script가 <head>에 놓인 경우(</deck-stage> 이후가 아닌),
      // 파서가 이 시점에 아직 자식 <section>을 처리하지 않았을 수 있어 querySelectorAll이 빈 결과를 반환할 수 있다.
      // 다음 이벤트 루프로 지연하여 자식 노드들이 모두 파싱되었는지 보장
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // 문서가 아직 파싱 중이면 DOMContentLoaded를 기다려 모든 section을 한 번에 처리
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // 문서가 이미 파싱 완료된 경우 (script가 body 하단에 있거나 defer), 다음 프레임에서 수집하면 됨
